...
export function getSchema(object: { [key: string]: FieldConfiguration }, path: string, defaultValue = null): { [key: string]: FieldConfiguration } | null {
    // Convert dot notation to bracket notation
    path = path.replace(/\[(\w+)\]/g, ".$1");
    path = path.replace(/^\./, "");
 ...
 *
 */
export const shapeCss = <Keys extends string, OverridesMap extends Partial<Record<Keys, string>>>(
    ...
    (Object.keys(overrides) as Keys[]).forEach(o => {
        obj[o] = combineCss([styles[o], __cssFor ? __cssFor[o as Keys] : ""]) as OverridesMap[Keys];
    });
    return obj;
};
