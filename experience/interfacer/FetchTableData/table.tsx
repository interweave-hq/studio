...
import { type InterfaceConfiguration, type FieldConfiguration } from "@interweave/interweave";
...
const getActions = ({ activeRow, onUpdate, onDelete }: { activeRow?: any; onDelete?: () => void; onUpdate?: () => void }) => {
...
const getColumnsFromKeys = (columnData: { [key: string]: FieldConfiguration }) => {
    ...
    const emptyCols = new Array(numEmptyColumns).fill({ header: "", key: null }).map((e, i) => ({ ...e, id: `empty-col-${i}` }));
...
export default function DynamicTable({
    ...
    const { data: cols, error, initialState } = useMemo(() => getColumnsFromKeys(columnData), []);
    ...
    const supplementalInfo = [`GET ${uri}`, requestDuration ? `Fetched ${data.length} entries in ${getDurationNumber(requestDuration)}` : ""];
    if (cols === null || error) {
        return (
            <Error
