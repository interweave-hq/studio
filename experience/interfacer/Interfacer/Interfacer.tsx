...
import { type InterfaceConfiguration, validate, type Fields } from "@interweave/interweave";
import { type Interfacer, type Error as ErrorType, type VariableState } from "@/interfaces";
...
export function Interfacer({
    ...
    const hasSubmissionError = submissionError?.technicalError || submissionError?.userError;

    const getComponentsFromKeys = (keys: Fields, nestedPath?: string): ComponentSetup[] => {
        const keysArr = Object.keys(keys);
        const determinedComponents = keysArr.map(k => {
            let typeConfig = keys[k];
            const formKey = nestedPath ? `${nestedPath}.${k}` : typeConfig.interface?.form?.out_key ? typeConfig.interface?.form?.out_key : k;
            ...
            if (type === "object") {
                if (typeConfig?.schema?.object_schema?.fields) {
                    return getComponentsFromKeys(typeConfig.schema?.object_schema, formKey);
                }
            }
            ...
            return GetComponent(
                formKey,
                {
                    ...
                    maxLength: getValueFromValidationOption(typeConfig?.validation?.max_length),
                },
                { variables }
            );
        });
        return determinedComponents.filter(v => v !== null).flat() as ComponentSetup[];
    };
    ...
    const onSubmit = async (data: Record<string, unknown>) => {
        ...
        const { data: submitData, error } = await clientRequest(`/api/v1/interfaces/${interfaceId}`, {
            method: "POST",
            requestBody: {
                method: "create",
                ...variables,
    ...
    return (
        <form
            ...
            autoCorrect="off"
        >
            {comps.map(({ component, key }) => cloneElement(component, { key }))}
            <Button
            ...
            {formSuccess ? <p className={styles["submission-success"]}>Data successfully saved!</p> : null}
        </form>
    );
}
