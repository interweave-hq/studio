...
export function DeleteAPITokens({ interfaceId, projectId }: { interfaceId: string; projectId: string }) {
    const router = useRouter();
    const onDelete = async () => {
        const { error } = await clientRequest(`/api/v1/projects/${projectId}/third-party-tokens`, { method: "DELETE", requestBody: { interfaceId } });
