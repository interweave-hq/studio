...
import { Column, Table as ReactTable, useReactTable, getCoreRowModel, getFilteredRowModel, getPaginationRowModel, flexRender } from "@tanstack/react-table";
import { useEffect, useState } from "react";
import styles from "./table.module.css";
import { Input } from "@/components/Input";
import { Beaker, ChevronDoubleLeft, ChevronDoubleRight, ChevronLeft, ChevronRight, Reload } from "@/components/Icons";
...
export default function Table({
    ...
    const [previousPaginationRestateValue, setPreviousPaginationRestateValue] = useState(restatePagination);
    ...
    const [previousPurgeRowState, setPreviousPurgeRowState] = useState(purgeRowState);
                ...
                <table className={styles.table}>
                                    ...
                                    return (
                                        ...
                                        >
                                            {header.isPlaceholder ? null : (
                                                <div className={styles["table__header-container"]}>
                    ...
                    <tbody>
                        {table.getRowModel().rows.map(row => {
                            ...
                            const className = selectedRow ? combineCss([styles.table__row, styles["table__row--selected"]]) : styles.table__row;
                            return (
                                <tr
                                    key={row.id}
                                    className={className}
                                    ...
                                    {row.getVisibleCells().map(cell => {
                                        return (
                                            <td
                                                ...
                                                key={cell.id}
                                            >
                                                <div className={styles["table__cell-text"]}>
            ...
            <div className={styles["table__bottom-container"]}>
                <div className={styles["table-controls"]}>
                    <div className={styles["control-group"]}>
                        ...
                        >
                            <ChevronDoubleLeft className={styles["table-controls__button-svg"]} />
                        </button>
                        <button
                            ...
                            className={styles["table-controls__button"]}
                        >
                            <ChevronLeft className={styles["table-controls__button-svg"]} />
                        </button>
                        <button
                            ...
                            className={styles["table-controls__button"]}
                        >
                            <ChevronRight className={styles["table-controls__button-svg"]} />
                        </button>
                        <button
                            ...
                            className={styles["table-controls__button"]}
                        >
                            <ChevronDoubleRight className={styles["table-controls__button-svg"]} />
                        </button>
                    </div>
                    <div className={styles["control-group"]}>
                        <span>
                            <p className={styles["table-controls__page-text"]}>Page</p>
                            <strong className={styles["table-controls__page-range-number"]}>
                    ...
                    <div className={styles["control-group"]}>
                        {reload ? (
                            <button
                                onClick={() => reload()}
                                className={combineCss([styles["table-controls__button"], styles["table__reload-button"]])}
                            >
                                <Reload className={combineCss([styles["table-controls__button-svg"], styles["table__reload-button-svg"]])} />
                            </button>
                        ) : null}
                        {actions ? <div className={styles["table__actions-container"]}>{actions.map(el => el)}</div> : null}
                    </div>
                </div>
                <div>
...
function FilterButton({ hidden, setHidden }: { hidden: boolean; setHidden: (value: boolean) => void }) {
            ...
            <div>
                <Input
                    ...
                    domProps={{
                        ...
                        onChange: (e: any) => {
                            column.setFilterValue((old: [number, number]) => [e.currentTarget.value, old?.[1]]);
                        },
                        placeholder: `1`,
                    }}
                />
                <Input
                    ...
                    domProps={{
                        ...
                        onChange: (e: any) => {
                            column.setFilterValue((old: [number, number]) => [old?.[0], e.currentTarget.value]);
                        },
                        placeholder: `100`,
                    }}
                />
