...
export type ModalOverrides<T> = InternalModalOverrides<T> &
	ExternalModalOverrides<T>;
...
export function Modal({
	...
	const [playingClosingAnimation, setPlayingClosingAnimation] =
		useState(false);
	...
	const {
		...
		return shapeCss<OverridesKeys, ModalOverrides<string>>(
			Overrides,
			styles,
			__cssFor
		);
	...
	const stylesOverlay = playingClosingAnimation
		? combineCss([styles.overlay, styles["overlay--closing"]])
		: styles.overlay;
	const stylesRoot = playingClosingAnimation
		? combineCss([rootStyles, styles["dialog--closing"]])
		: rootStyles;
	...
	return (
		<div>
			...
			<div className={stylesRoot}>
				<div className={dialogStyles} role="dialog" {...ariaProps}>
					{isDismissible ? (
